...
public class BlockingAnEventTest extends LaunchWebBrowser {
	...
	private static void VeriBlockingAnEvent(boolean isAlreadyLogIn2) throws Exception {
		...
		int ayear = calendar.get(Calendar.YEAR);
			...
			List<WebElement> list=driver.findElements(By.xpath("//table[@class='calDays']//tr"));
			for(int i=0; i<list.size(); i++){
				if(driver.findElement(By.xpath(list.get(i)+"//td[matches(text(),"+aDay+")]")).getText().equals(aDay)) {
					WebElement sday=driver.findElement(By.xpath("//table[@class='calDays']//tr//td[contains(text(),'"+aDay+"')]"));
					sday.click();
				}
			}
		String xpathDate="//td[contains(text(),'"+aDay+"')]";
		WebElement twoWeeksADay = driver.findElement(By.xpath(xpathDate));
		twoWeeksADay.click();
		Thread.sleep(2000);
